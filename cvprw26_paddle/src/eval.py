"""Self-contained server-side evaluation script for holdout predictions (PaddlePaddle version).

This script is framework-agnostic - it only uses pycocotools for COCO evaluation
and does not depend on PaddlePaddle at all. It is identical to the PyTorch version.
"""
...
def _load_coco_gt(gt_path: str) -> tuple:
...
def _build_zero_report(coco_gt: COCO) -> dict:
    report = {"mAP": 0.0, "AP50": 0.0, "AP75": 0.0}
    for cat_id in coco_gt.getCatIds():
        report[coco_gt.cats[cat_id]["name"]] = 0.0
    return report


def _build_server_report(coco_eval: COCOeval) -> dict:
...
def _evaluate_segmentation(coco_gt: COCO, prediction_list: list) -> dict:
